...
from design_c import Ui_MainWindow
...
class MyWidget(QMainWindow, Ui_MainWindow):
    ...
    def run(self):
        ...
        if f == 'Не выбрано':
            data = self.cur.execute('SELECT * FROM Films').fetchmany(200)
        else:
            ID = self.cur.execute(f'SELECT * FROM Genres WHERE title = \'{f}\'').fetchall()
            data = self.cur.execute(f'SELECT * FROM Films WHERE genre = \'{ID[0][0]}\'').fetchmany(20)
